const sdk = require('node-appwrite');

const client = new sdk.Client();
client
    .setEndpoint('https://sgp.cloud.appwrite.io/v1')
    .setProject('69ca6dd30013a519ec48')
    .setKey('standard_');

const databases = new sdk.Databases(client);
const DB_ID = 'ledger-db';

async function check() {
    try {
        const col = await databases.getCollection(DB_ID, 'accounts');
        console.log("Collection info:");
        console.log("- documentSecurity:", col.documentSecurity);
        console.log("- $permissions:", col.$permissions);
        
        // Let's create a test document as ADMIN
        console.log("Creating test doc as admin...");
        await databases.createDocument(DB_ID, 'accounts', 'unique()', {
            name: "Test",
            type: "bank",
            currency: "KRW",
            initialBalance: 0
        });
        console.log("Admin Create OK!");
    } catch(e) {
        console.error("Error:", e.message);
    }
}
check();
